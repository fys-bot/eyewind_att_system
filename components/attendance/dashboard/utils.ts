...
export class DashboardCache {
    ...
    /**
     * 获取仪表盘数据（忽略过期检查）
     * 用于月度环比等场景，需要读取历史月份的缓存数据
     */
    static async getDashboardDataIgnoreExpiry(company: string, yearMonth: string): Promise<{
        employees: DingTalkUser[];
        companyCounts: CompanyCounts;
        processDataMap: Record<string, any>;
        attendanceMap: AttendanceMap;
        ...
        try {
            ...
            return new Promise((resolve) => {
                ...
                const store = transaction.objectStore(STORE_NAME);
                const request = store.get(CACHE_PREFIX + cacheKey);
                request.onsuccess = () => {
                    const item = request.result;
                    if (!item || !item.data) { resolve(null); return; }
                    resolve(item.data);
                };
